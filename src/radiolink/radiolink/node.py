...
class Gears(Enum):
    FIRST = GearConfig(0.3, 0.2)
    SECOND = GearConfig(0.6, 0.4)
    THIRD = GearConfig(1.0, 0.8)
...
class RadiolinkNode(Node):
    ...
    def joy_callback(self, data: Joy):
        ...
        # Debug the joystick values
        self.get_logger().info(f"Joystick axes: {data.axes}")

        linear_x = 0
        # Check if the joystick is being pushed forward or backward
        # For most controllers, axes[1] is the left stick up/down
        if abs(data.axes[1]) > threshold:
            # Invert the sign to make pushing up go forward
            linear_x = math.copysign(min(abs(data.axes[1]), max_linear_vel), data.axes[1]) * self.max_linear_speed
